		...
		for (k in 1:2) { # 2 cat-lev types
			for (l in 1:2) { # 2 balance levels
				for (m in 1:2) { # 100 datasets
					...
					data_4800[[i]][[j]][[k]][[l]][[m]]$kamila$ARI <- round(ARI(z$lbl, z_kam$finalMemb), 3)
					...
					data_4800[[i]][[j]][[k]][[l]][[m]]$kproto$ARI <- round(ARI(z$lbl, z_kproto$cluster), 3)
